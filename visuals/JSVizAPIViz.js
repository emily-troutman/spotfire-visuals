// Visualise using a API Visualization

// Custom Visualization Draft using JSVizNetwork.js file as foundation

/*
Goal for Visualization Functionality:

Undirected unweighted graph data structure
    Nodes/Vertices are Systems or API
    Edges are connections between Systems and API

Nodes are color coded to reflect type: System, exp API, proc API, sys API or other
Edges are labeled to show endpoint information for connection

Each API has the following information to be displayed either in tooltip or otherwise:
id, name, endpoints and methods, type, MuleSoft Link

Want to use the visualization so user can search for API and their connections

Ways to search:
    Find all possible API connecting two Systems - To: System From: System
    Find API by name or id
    Filter by Node Type
    Find API by endpoint and method information

When found, the results for the search should be highlighted

Layers reveal more detail to user when activated:
    When mouseover API, tooltip reflecting information appears
